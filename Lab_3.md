...
В рамках MVP размещение системы в двух дата-центрах (DC1 и DC2).
Цель — сохранить доступность сервиса при потере одного дата-центра и минимизировать потерю данных.

### Размещение компонентов
- Компоненты без сохранения состояния (API Gateway, Feed Service, Post Service, User Service, Notification Service) запускаются в обоих DC.
  Они не хранят состояние и могут быть быстро перезапущены или масштабированы.
- Redis не хранит исходные данные, он хранит только временную копию. Его потеря приводит к росту нагрузки на БД, но не к потере данных.
- PostgreSQL является источником исходных данных. Для отказоустойчивости используется репликация.

### Репликация PostgreSQL
Выбран вариант: **Основной - DC1 + резервный (реплика) - DC2**.
- Основной принимает записи.
- Реплика получает изменения через репликацию.
- При отказе DC1 выполняется failover: реплика в DC2 становится новым основным источником данных.

Возможные режимы репликации:
- **Async replication**: работает, как правило, быстрее, но при падении DC1 возможна небольшая потеря последних записей.
- **Sync replication**: обычно надёжнее по данным, но медленнее по записи.

Для MVP возьмем режим async с оговоркой, что потеря данных ограничена последними секундами/транзакциями.

### Что происходит при отказе одного DC
### Если DC1 недоступен:
1. Глобальный балансировщик (DNS/LB) переключает трафик на DC2.
2. Резервные-сервисы продолжают обслуживать запросы в DC2.
3. Реплика PostgreSQL в DC2 повышается до основной (failover).
4. Redis-кеш в DC1 теряется, но кеш в DC2 продолжает работу.
5. Пользователи продолжают получать ленту и создавать посты в DC2.

Если позже DC1 возвращается:
- Он становится вторичным и догоняет данные репликацией.
- После синхронизации возможно плановое переключение ролей (failback), если требуется.

### Что с данными в БД при отказе
- Источник исходных данных — PostgreSQL.
- В async-режиме возможна небольшая потеря последних транзакций, которые не успели попасть на реплику.
- В sync-режиме потеря данных минимальна, но возрастает задержка записи.

### Схема 4 - Демонстрация сценария отказа основного DC
<img width="1520" height="624" alt="Отказ DC" src="https://github.com/user-attachments/assets/73581344-16f1-46c6-869f-13ba5e0c46e0" />

### Если DC2 недоступен:
1. Redis - не источник данных, данные остаются на DC1
2. Система не падает и мы ничего не теряем, но система начинает работать медленнее

Если позже DC2 возвращается:
Данные на нем догоняются репликацией

...
<img width="745" height="651" alt="Архитектура1_маркетплейс" src="https://github.com/user-attachments/assets/f69803c2-9894-418c-90ba-357dca08f0d2" />
...
<img width="1620" height="725" alt="Отказ4_маркетплейс (1)" src="https://github.com/user-attachments/assets/e3e8c4fd-9f6c-4c92-a995-6dce204fa9d1" />

При отказе основного дата-центра (DC1) Global Load Balancer
перенаправляет пользовательский трафик в резервный DC2.
PostgreSQL Replica в DC2 содержит реплицированные данные и может
быть повышена до Primary.

Redis используется только как кеш и не реплицируется между дата-центрами.
После failover кеш в DC2 восстанавливается постепенно на основе запросов
к базе данных, что приводит к временной деградации производительности,
но не к потере данных.
